...
if __name__ == "__main__":
    ...
    if X is not None and len(X) > 0:
        ...
        print("=== Training Lasso Regression with Nested LOOCV ===")
        ...
        grid_search = GridSearchCV(lasso, param_grid, cv=3, scoring='neg_mean_squared_error', n_jobs=1)
        
        # Outer CV: Leave-One-Out
        loo = LeaveOneOut()
        y_pred_lasso = cross_val_predict(grid_search, X, y, cv=loo, n_jobs=-1)
        
        rmse_lasso = np.sqrt(mean_squared_error(y, y_pred_lasso))
        r2_lasso = r2_score(y, y_pred_lasso)
        
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{timestamp}] Lasso LOOCV Result - RMSE: {rmse_lasso:.4f}, R2: {r2_lasso:.4f}")
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{timestamp}] Lasso Result - RMSE: {rmse_lasso:.4f}, R2: {r2_lasso:.4f}")
