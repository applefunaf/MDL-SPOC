...
if __name__ == "__main__":
    ...
    if X is not None and len(X) > 0:
        ...
        print("=== Training Random Forest with Nested LOOCV ===")
        ...
        loo = LeaveOneOut()
        # n_jobs=-1 to parallelize the outer loops
        y_pred_rf = cross_val_predict(grid_search, X, y, cv=loo, n_jobs=-1)
        
        rmse_rf = np.sqrt(mean_squared_error(y, y_pred_rf))
        r2_rf = r2_score(y, y_pred_rf)
        
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{timestamp}] Random Forest LOOCV Result - RMSE: {rmse_rf:.4f}, R2: {r2_rf:.4f}")

        # Final fit on all data for production use (optional)
